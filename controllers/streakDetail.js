...
const modifyingStreakDetail = asyncHandler(async (streakDetail) => {
  ...
  const filterRewardList = rewards.filter((reward) => {
    ...
    if (
      modifiedReward.streakId === streakDetail.streakId &&
      moment(moment(modifiedReward.date).format("YYYY-MM-DD")).isSame(
        moment(streakDetail.date).format("YYYY-MM-DD")
      )
    ) {
      return reward;
    }
  ...
  async () => {
    try {
      ...
      filterStreakData.map(async (detail) => {
        ...
        const streakDetail = await StreakDetail.find({
          streakId: detail.id,
        }).lean();
        //Streak Detail will be [] if it was the case of upcoming streak
        const lastStreakDetail =
          streakDetail.length > 0
            ? streakDetail[streakDetail.length - 1]
            : { description: "" }; //if lenth of streak detail is 0 then it must be first streak detail
        ...
        if (streakDetail.length <= +detail.days) {
          //it means streak have capabilites of creating more streak details
          if (descriptionOfLast.length > 0 || streakDetail.length === 0) {
            //last streak detail have some description or it is first streak detail
            ...
            await Streak.findByIdAndUpdate(
              { _id: mongoose.Types.ObjectId(streakId) },
              updateObj,
              { new: true }
            );
            ...
            const rewards = await Reward.find({
              streakId: mongoose.Types.ObjectId(streakId),
            }).lean();
            ...
            if (rewards.length > 0) {
              ...
              // io.emit("data-update", { message: "Initial data from server" });
            }
          }
        } else {
          //STREAK IS FINISHED
        ...
        await Promise.all(
          rewards.map(async (reward) => {
            ...
            if (
              moment(moment(reward.date).format("YYYY-MM-DD")).isSameOrBefore(
              ...
              await Reward.findByIdAndUpdate(
                { _id: mongoose.Types.ObjectId(reward._id) },
                updatedReward,
                { new: true }
              );
            }
...
export const updateStreakDetail = asyncHandler(async (req, res, next) => {
  ...
  const updatedStreakDetail = await StreakDetail.findByIdAndUpdate(
    _id,
    streakDetail,
    { new: true }
  );
  if (!updatedStreakDetail)
    throwError(404, `Streak Detail not found with id ${_id}`, next);
